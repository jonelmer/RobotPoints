...
class LaserTestCase(unittest.TestCase):
    ...
    def test_interpolate_grid(self):
        laser = Laser(3., 3., 5.)
        path = PathFinder(laser, trimesh.Trimesh(), process=False)
        ass = DeviationAssessor(path, resolution=3, process=False)
        target = Target((1.5, 0., 1.5), (0., 1., 0.))
        grid = ass.interpolate_grid(target)
